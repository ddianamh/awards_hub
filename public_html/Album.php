...
if (isset($_GET['album_id'])) {
    ...
    if ($id > 10000) {
        $sql = $conn->prepare("SELECT album_id, album_title, version, genre, language, name, surname, is_winner_album
            FROM Album, Music, CREATES, Singer
            WHERE CREATES.music_id = Album.album_id
            AND CREATES.artist_id = Singer.singer_id 
            AND Album.album_id = Music.music_id
            AND Album.album_id = ?");
        $sql->bind_param("i", $id);
        $sql->execute();
        $result = $sql->get_result();

        $stmt = $conn->prepare("SELECT album_id, album_title, version, genre, language, band_name, is_winner_album
            FROM Album, Music, CREATES, Band
            WHERE CREATES.music_id = Album.album_id
        ...
        $sql2 = $conn->prepare("SELECT album_id, album_title, version, genre, language, is_winner_album
                FROM Album, Music
                WHERE Album.album_id = Music.music_id
                AND Album.album_id = ?");
    ...
    // 'album_id' is not set, handle it accordingly (redirect, show an error, etc.)
    die("No 'album_id' provided in the URL.");
}
?>
...
<body>
    ...
    Here is more information about the selected album:
	</h2>
        ...
        <table style="width:80%">
            ...
            if ($id > 10000) {
                ...
                while ($rows1 = $result1->fetch_assoc()) {
                    ?>
                    <tr>
                        <td><?php echo $rows1['album_title'];?></td>
                ...
                while ($rows2 = $result2->fetch_assoc()) {
                    ?>
                    <tr>
                        <td><?php echo $rows2['album_title'];?></td>
